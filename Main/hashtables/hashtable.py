...
class HashTable:
    ...
    def encode(key: T) -> int:
        ...
        if isinstance(key, str):
            ...
            raise Exception(
                f"Cannot encode {type(key)} (Only strings and integers are supported)")
    ...
    def update(self, key: T, value: T) -> None:
        ...
        if self.table[idx] is None:  # idx location not occupied
            ...
            if self.table[idx][0] == key:  # trying to insert to the same key
                # update 'value' at 'key'
                self.table[idx] = (self.table[idx][0], value)
            else:
                ...
                self.table[(idx + i * idx2) % self.table_size] = (key, value)
                self.filled_count += 1
...
if __name__ == "__main__":
    ts: List[str] = ["the", "of", "and", "to", "a", "in", "for", "is", "on", "that", "by", "this", "with", "i", "you", "it", "not", "or", "be", "are", "from", "at", "as", "your", "all", "have",
                     "new", "more", "an", "was", "we", "will", "home", "can", "us", "about", "if", "page", "my", "has", "search", "free", "but", "our", "one", "other", "do", "no", "information", "time"]
